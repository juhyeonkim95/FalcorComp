    ...
    : RenderPass(pDevice)
{
    ...
    mpCounters = mpDevice->createBuffer((size_t)Counters::kCount * sizeof(uint32_t), Resource::BindFlags::ShaderResource | Resource::BindFlags::UnorderedAccess, MemoryType::DeviceLocal);
    mpCountersReadback = mpDevice->createBuffer((size_t)Counters::kCount * sizeof(uint32_t), Resource::BindFlags::None, MemoryType::ReadBack);
...
Properties ReSTIRPTPass::getSpecializedScriptingDictionary()
{
    Properties d;
    d[kMaxSurfaceBounces] = mStaticParams.maxSurfaceBounces;
    d[kSpatialMisKind] = mStaticParams.spatialMisKind;
...
void ReSTIRPTPass::prepareResources(RenderContext* pRenderContext, const RenderData& renderData)
{
    ...
    if (mStaticParams.pathSamplingMode != PathSamplingMode::PathTracing)
    {
        ...
        {
            mReconnectionDataBuffer = mpDevice->createStructuredBuffer(var["reconnectionDataBuffer"], reservoirCount, Resource::BindFlags::ShaderResource | Resource::BindFlags::UnorderedAccess, MemoryType::DeviceLocal, nullptr, false);
        ...
        {
            mpOutputReservoirs = mpDevice->createStructuredBuffer(var["outputReservoirs"], reservoirCount, Resource::BindFlags::ShaderResource | Resource::BindFlags::UnorderedAccess, MemoryType::DeviceLocal, nullptr, false);
            ...
            if (mStaticParams.pathSamplingMode != PathSamplingMode::PathReuse)
            {
                ...
                for (uint32_t i = 0; i < mStaticParams.samplesPerPixel; i++)
                    mpTemporalReservoirs[i] = mpDevice->createStructuredBuffer(var["outputReservoirs"], reservoirCount, Resource::BindFlags::ShaderResource | Resource::BindFlags::UnorderedAccess, MemoryType::DeviceLocal, nullptr, false);
            }
            mVarsChanged = true;
        }
        ...
        if (mStaticParams.pathSamplingMode == PathSamplingMode::PathReuse)
        {
            ...
            {
                mPathReuseMISWeightBuffer = mpDevice->createStructuredBuffer(var["misWeightBuffer"], reservoirCount, Resource::BindFlags::ShaderResource | Resource::BindFlags::UnorderedAccess, MemoryType::DeviceLocal, nullptr, false);
                mVarsChanged = true;
            }
