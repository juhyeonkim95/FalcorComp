...
def parse_args():
    ...
    parser.add_argument("--scene", type=Path, default=ROOT / "scenes/exp1/cornell_box.json", help="Scene JSON; asset paths are relative to this JSON")
...
def main():
    args, scene, budget = parse_args()
    output = args.output.resolve()
    spatial = SpatialOptions(neighbors=args.neighbors, iterations=args.iterations, radius_pixels=args.radius)
    existing_reference, reference_source = find_reference(output, scene, args.reference, args.regenerate_reference)
    testbed = create_testbed(scene)
    clear_comparison_outputs(output, keep_reference=existing_reference is not None)
    manifest = {
        "experiment": "offline_rendering_with_spatial_reuse_comparison", "scene": asdict(scene),
        "budget": asdict(budget), "spatial": asdict(spatial), "methods": {**METHODS, "ours": {**METHODS["ours"], "shiftmapMethod": scene.shiftmap_method}},
        "initial_sampling": "direct", "temporal_reuse": False,
    ...
    for method in METHODS:
        graph = create_graph(testbed, method, scene, budget.spp_per_frame, spatial)
        method_rows, method_times, warmup_times, frames = render_method(testbed, graph, method, budget, output)
    ...
    save_image(output / "reference.npy", reference)
    save_json(output / "reference.json", metadata)
    errors = evaluate_run(output)
    ...
    print("\nmethod   spp       seconds    relative_RMSE  MAPE (fraction)")
    for row in errors:
        print(f"{row['method']:7s}  {int(row['spp']):8d}  {float(row['elapsed_seconds']):8.3f}  "
              f"{row['relative_rmse']:13.6g}  {row['mape']:.6g}")
